# @kelvdra/bails - WhatsApp Web API Documentation
...
**Weekly Downloads:** 109

A modern and feature-rich WhatsApp Web API built on top of Baileys — designed for automation, chatbots, and integration with any Node.js project.
...
- [🔌 Connecting Account](#-connecting-account)
  ...
  - [Connect With Number (Pairing Code)](#connect-with-number)
...
- [✉️ Sending Messages](#sending-messages)
  ...
  - [Interactive Messages](#interactive-message-with-native-flow)
...
Or check the [example.ts](https://github.com/WhiskeySockets/Baileys/blob/HEAD/Example/example.ts) for a TypeScript version.
...
WhatsApp supports multi-device API, allowing Baileys to act as a secondary WhatsApp Web client. You can connect via QR Code or Pairing Code.

### With QR Code

```javascript
const { default: makeWASocket } = require('@kelvdra/bails');
...
```javascript
const { default: makeWASocket, fetchLatestWAWebVersion } = require('@kelvdra/bails');
...
💡 **Recommended:** Store auth in a database for production.
...
By default poll votes are encrypted and handled in `messages.update`. Here's a simple example:
...
- [AnyMessageContent](https://baileys.whiskeysockets.io/types/AnyMessageContent.html) - all message contents supported
- [MiscMessageGenerationOptions](https://baileys.whiskeysockets.io/types/MiscMessageGenerationOptions.html) - all options supported
...
You need to have message object, can be retrieved from store or use a [message](https://baileys.whiskeysockets.io/types/WAMessage.html) object
...
You need to pass the key of message, you can retrieve from store or use a [key](https://baileys.whiskeysockets.io/types/WAMessageKey.html) object
...
3. To enable this function's usage, add `link-preview-js` as a dependency: `yarn add link-preview-js`
4. Send a link:
...
> [!NOTE]
> In media messages, you can pass `{ stream: Stream }` or `{ url: Url }` or `Buffer` directly. See more [here](https://baileys.whiskeysockets.io/types/WAMediaUpload.html)
...
To audio message work in all devices you need to convert with some tool like `ffmpeg` with these flags:
...
Note: deleting for oneself is supported via `chatModify`, see in the Modifying Chats section
...
- For media messages, the thumbnail can be generated automatically for images & stickers provided you add `jimp` or `sharp` as a dependency: `yarn add jimp` or `yarn add sharp`
...
sock.ev.on('messages.upsert', async ({ messages: [m] }) => {
    ...
    const messageType = getContentType(m) // get what type of message it is
...
A set of message [keys](https://baileys.whiskeysockets.io/types/WAMessageKey.html) must be explicitly marked read now. You cannot mark an entire 'chat' read as it were with Baileys Web. This means you have to keep track of unread messages.
...
`presence` can be one of [these](https://baileys.whiskeysockets.io/types/WAPresence.html). The presence expires after about 10 seconds. This lets the person/group with `jid` know whether you're online, offline, typing etc.
...
> [!NOTE]
> Like media messages, you can pass `{ stream: Stream }` or `{ url: Url }` or `Buffer` directly. See more [here](https://baileys.whiskeysockets.io/types/WAMediaUpload.html)
...
baileys, baileys-mod, bails, whiskeysockets/baileys, whatsapp, meta, Hydra, Kelvdra, js-whatsapp, whatsapp-api, whatsapp-web, whatsapp-chat, whatsapp-group, automation, multi-device, modder
...
**Repository:** [github.com/WhiskeySockets/Baileys](https://github.com/WhiskeySockets/Baileys)

**Homepage:** [github.com/WhiskeySockets/Baileys/](https://github.com/WhiskeySockets/Baileys/)

**Weekly Downloads:** 109  
**Version:** 1.4.7  
**License:** MIT  
**Unpacked Size:** 41.4 MB  
**Total Files:** 284  
**Last publish:** 2 days ago

**Collaborators:** [hydraxyz46](https://www.npmjs.com/~hydraxyz46)
...
*Documentation fetched from: https://www.npmjs.com/package/@kelvdra/bails*
